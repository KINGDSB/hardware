...
public interface ProductMapper extends Mapper<Product> {
	...
	@Select("<script>"
			...
			+ " t1.picture, t1.pictures, t1.description, "
			...
			+ " FROM f_product t1 "
			+ " LEFT JOIN f_product_series t2 ON t2.id = t1.type AND t2.is_deleted = 0 "
...
//			+ " AND t1.type = #{type}" 
			+ " AND (t1.type = #{type} OR t2.parent_id = #{type}) " 
			+ "</if>"
    ...
    @Select(" SELECT "
        + " t1.id, t1.name_cn AS 'nameCn', t1.name_en AS 'nameEn', "
        + " t1.picture, t1.pictures, t1.description, t1.specification, "
